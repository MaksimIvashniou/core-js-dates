...
function getNextFriday(date) {
  let daysToFriday = 5 - date.getDay();
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor(Date.parse(dateEnd) - Date.parse(dateStart)) / dayInMs + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 29; day <= daysCount; day += 1) {
...
function getWeekNumberByDate(date) {
  ...
  const timestamp = date - new Date(date.getFullYear(), 0, 1);
  return Math.floor(timestamp / weekInMs) + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  if (date.getDate() >= searchParams.day) {
    setNextMonth();
  }

  date.setDate(searchParams.day);

  while (date.getDay() !== searchParams.weekday) {
    setNextMonth();
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const dmyParser = (date) => date.split('-').reverse().join('-');

  const workDay = new Date(dmyParser(period.start));
  const endDate = new Date(dmyParser(period.end));

  let workDaysCounter = countWorkDays;

  do {
    workDaysCounter -= 1;
    let increment = 0;

    const output = workDay.toISOString().slice(0, 10);
    workSchedule.push(dmyParser(output));

    if (!workDaysCounter) {
      workDaysCounter = countWorkDays;
      increment = countOffDays;
    }

    workDay.setDate(workDay.getDate() + 1 + increment);
